...
fn test_prelude_exports_mcp_types() {
    ...
    // Note: This test is a compile-time check and doesn't actually call McpToolAdapter::new
    // because it would require async context and a dummy client
...
async fn test_mcp_client_url_and_command_both_set() {
    ...
    // Command is preferred when both are set, so it tries stdio transport.
    let result = McpClient::connect(config).await;
    // This will fail because npx without args won't spawn properly,
    // but the point is it tries to use the command (not the URL).
...
#[tokio::test]
async fn test_tool_registry_mixed_builtins_and_mcp() {
...
#[tokio::test]
async fn test_mcp_tool_adapter_complex_schema() {
    let schema = json!({
        "type": "object",
