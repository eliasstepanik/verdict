...
async fn test_http_tool_network_deny_all() {
    ...
    assert!(
        result.is_err(),
        "Should reject network call with DenyAll policy"
    );

    if let Err(ToolError::ExecutionFailed { reason }) = result {
        assert!(
            reason.contains("network policy"),
...
async fn test_self_update_invalid_patch_fails() {
    ...
    let result =
        SelfUpdateEngine::apply_in_sandbox(garbage_patch, tempdir.path(), tempdir.path()).await;

    assert!(result.is_err(), "Invalid patch should fail: {:?}", result);
}
