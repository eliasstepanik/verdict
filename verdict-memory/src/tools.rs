...
use verdict::tools::{Tool, ToolContext, ToolError, ToolOutput, ToolSource};
...
impl Tool for MemoryGetThreadTool {
    ...
    async fn call(&self, args: Value, _ctx: ToolContext) -> Result<ToolOutput, ToolError> {
        let thread_id =
            args["thread_id"]
                ...
                .ok_or_else(|| ToolError::SchemaValidationFailed {
                    reason: "thread_id is required".to_string(),
                })?;
        ...
        let output = serde_json::to_string(&messages).map_err(|e| ToolError::ExecutionFailed {
            reason: e.to_string(),
        })?;
...
impl Tool for MemorySearchTool {
    ...
    async fn call(&self, args: Value, _ctx: ToolContext) -> Result<ToolOutput, ToolError> {
        let embedding = args["embedding"]
            .as_array()
            .ok_or_else(|| ToolError::SchemaValidationFailed {
                reason: "embedding must be an array".to_string(),
            })?
            .iter()
            .map(|v| {
                v.as_f64().ok_or_else(|| ToolError::SchemaValidationFailed {
                    reason: "embedding values must be numbers".to_string(),
                })
            })
        ...
        let top_k = args["top_k"]
            .as_u64()
            .ok_or_else(|| ToolError::SchemaValidationFailed {
                reason: "top_k is required".to_string(),
            })? as usize;
        ...
        let output = serde_json::to_string(&results).map_err(|e| ToolError::ExecutionFailed {
            reason: e.to_string(),
        })?;
    ...
    async fn call(&self, args: Value, _ctx: ToolContext) -> Result<ToolOutput, ToolError> {
        let resource_id =
            args["resource_id"]
                .as_str()
                .ok_or_else(|| ToolError::SchemaValidationFailed {
                    reason: "resource_id is required".to_string(),
                })?;
...
impl Tool for MemoryGetWorkingTool {
    ...
    async fn call(&self, args: Value, _ctx: ToolContext) -> Result<ToolOutput, ToolError> {
        ...
        let output = serde_json::to_string(&data).map_err(|e| ToolError::ExecutionFailed {
            reason: e.to_string(),
        })?;
...
impl Tool for MemorySaveMessageTool {
    ...
    async fn call(&self, args: Value, _ctx: ToolContext) -> Result<ToolOutput, ToolError> {
        ...
        let role_str = args["role"]
            .as_str()
            .ok_or_else(|| ToolError::SchemaValidationFailed {
                reason: "role is required".to_string(),
            })?;
