...
use verdict_memory::{InMemoryStore, ThreadMemory, WorkingMemory, SemanticMemory, ObservationalMemory};
use serde_json::json;
use std::sync::Arc;
...
async fn test_in_memory_save_get_thread() {
    ...
    for i in 0..5 {
        ...
        store.save_message("thread1", "resource1", msg).await.unwrap();
    }
...
async fn test_in_memory_last_n_limit() {
    ...
    for i in 0..10 {
        ...
        store.save_message("thread2", "resource2", msg).await.unwrap();
    }
...
async fn test_in_memory_working_memory() {
    ...
    store.save_working_memory("resource1", data.clone()).await.unwrap();
...
async fn test_in_memory_semantic_search() {
    ...
    store.upsert_embedding("id1", "text1", embed1.clone(), json!({"type": "A"})).await.unwrap();
    store.upsert_embedding("id2", "text2", embed2.clone(), json!({"type": "A"})).await.unwrap();
    store.upsert_embedding("id3", "text3", embed3.clone(), json!({"type": "B"})).await.unwrap();
...
async fn test_in_memory_observations() {
    ...
    // Save 3 observations
    store.save_observation("thread1", "Observation 1").await.unwrap();
    store.save_observation("thread1", "Observation 2").await.unwrap();
    store.save_observation("thread1", "Observation 3").await.unwrap();
...
async fn test_thread_memory_wrapper() {
    ...
    thread_mem.push("t1", "r1", MemoryRole::User, "Hello".to_string()).await.unwrap();
    thread_mem.push("t1", "r1", MemoryRole::Assistant, "Hi there".to_string()).await.unwrap();
...
async fn test_semantic_memory_wrapper() {
    ...
    semantic_mem.upsert("id1", "text", embedding.clone(), json!({})). await.unwrap();
